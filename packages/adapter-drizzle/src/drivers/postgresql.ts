...
> implements Adapter {
	...
	private get querySessionTable(): PostgreSQLSessionTable {
		return this.sessionTable;
	}
	private get queryUserTable(): PostgreSQLUserTable {
	...
	public async updateSessionExpiration(sessionId: string, expiresAt: Date): Promise<void> {
		await this.db
			...
			.set({ expiresAt } as Partial<PostgreSQLSessionTable["$inferInsert"]>)
			.where(eq(this.querySessionTable.id, sessionId));
	}
...
type PostgreSQLUserColumns = {
	id: AnyPgColumn<{ data: UserId }>;
};

type PostgreSQLSessionColumns = {
	id: AnyPgColumn<{ data: string }>;
	expiresAt: AnyPgColumn<{ data: Date }>;
	userId: AnyPgColumn<{ data: UserId }>;
