...
import { runAdapterContract, runOrphanContract, runVersionMismatchContract } from "./contract.js";
import "./register.js";
...
test(
	...
	async (context) => {
		...
		await pool.query("ALTER TABLE session ADD COLUMN token_version SMALLINT NOT NULL DEFAULT 1");
		const expanded = await pool.query("SELECT token_version, secret_hash FROM session WHERE id = $1", ["a".repeat(40)]);
		...
		await runVersionMismatchContract(adapter, async (sessionId) => {
			await pool.query("UPDATE session SET token_version = 1, secret_hash = NULL WHERE id = $1", [sessionId]);
		});
