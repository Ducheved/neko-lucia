...
async function prepareMigrated(connection: mysql.Connection): Promise<void> {
	...
	await connection.execute("ALTER TABLE session MODIFY id VARCHAR(66) CHARACTER SET ascii COLLATE ascii_bin NOT NULL");
	await connection.execute("ALTER TABLE session ADD COLUMN secret_hash BINARY(32) NULL");
	await connection.execute("ALTER TABLE session ADD COLUMN token_version INT NOT NULL DEFAULT 1");
	const [rows] = await connection.query("SELECT token_version, secret_hash FROM session WHERE id = ?", [
		"a".repeat(40),
	]);
...
test("MySQL expand migration and adapter contract", { skip: process.env.MYSQL_HOST === undefined }, async (context) => {
	...
	await runVersionMismatchContract(adapter, async (sessionId) => {
		await connection.execute("UPDATE session SET token_version = 1, secret_hash = NULL WHERE id = ?", [sessionId]);
	});
...
test("MySQL preserves arbitrary binary hashes", { skip: process.env.MYSQL_HOST === undefined }, async (context) => {
	const connection = await createConnection();
	context.after(() => connection.end());
